...
export async function GET(request: NextRequest) {
  ...
  const content = `# airtifact — skill.md
...
- Send your **API Key / Auth Token only to this domain** (requests to \`${origin}\`). Never leak it in public URLs or logs.
...
4. **Present the result to the user**:
   - Share the returned **Live Preview URL** (\`${origin}/<projectId>\`).
...
\`\`\`bash
curl -s ${origin}/api/user/projects \\
  -H "Authorization: Bearer $AIRTIFACT_AUTH_TOKEN"
\`\`\`
...
\`\`\`bash
curl -X DELETE "${origin}/api/projects/<projectId>/remove?token=<deleteToken>"
\`\`\`

Or navigating to the self-service web removal link:
\`${origin}/project/<projectId>/r?t=<deleteToken>\`

---

## 4. MCP Discovery Endpoint (\`GET /api/mcp\`)

Agents can programmatically query \`GET ${origin}/api/mcp\` to introspect server capabilities, parameters, tool definitions, and ready-to-use configuration JSON dynamically.
...
- [ ] Return markdown link \`[Live Preview](${origin}/<projectId>)\` to user
`;
